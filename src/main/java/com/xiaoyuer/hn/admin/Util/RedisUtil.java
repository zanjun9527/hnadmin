...
public class RedisUtil {
	...
	/**
	 * 操作spring-session 在redis中存储的session信息
	 */
	@Autowired(required = false)
     ...
     * 获取所有给定字段的值
//     */
    public List<Object> hMultiGet(String key, String... fields) {
     ...
     * @return
     */
    public  <T> T getFromSession(HttpServletRequest request,String key,Class<T> clazz){
      try {
	...
	public <T> T getObjectFromRedis (String key,Class<T> clazz){
		try {
				String keyString = get(key);
				if(!StringUtil.isBlank_new(keyString)) {
					return JSON.parseObject(keyString, clazz);
				}else {
					return null;
				}
			} catch (Exception e) {
			LOG.error("getObjectFromRedis_error",e);
				return null;
		}
	}
    ...
    /**
     * 	只用来获取登录标记，常规hset不要走这里
     * @param key
     * @param field
     * @return
     */
    public String hgetForSpringSession(String key, String field){
  	  String result  = (String)redisTemplate2.opsForHash().get(key, field);
  	  return result;
    }
    
    public  long hsetForSpringSession(String key, String fieid, String value) {
        HashOperations<Object, Object, Object> ho = redisTemplate2.opsForHash();
        ho.put(key,fieid,value);
        return 1L;
    }
    
    public String findDictMap(String key) {
    	return stringRedisTemplate.opsForValue().get(ConstantsRedis.DICT + ":" + key);
    }
    public String findDictMapHget(String key,String field) {
    	Object obj = stringRedisTemplate.opsForHash().get(ConstantsRedis.DICT + ":" + key, field);
